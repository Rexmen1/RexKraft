    ...
    for block_type in block_types:
        for i, milestone in enumerate(quests):
            ...
            actions = [
                ...
                "centered_message:  ",
                f"centered_message:  &eMine {milestone} {block_type} Blocks.",
                "centered_message:  ",
                "centered_message:  &aRewards:",
                f"centered_message:  &7- {milestone // 8}x Tokens",
                f"centered_message:  &7- {i + 1}x Random Mystery Box" if i >= 3 else "",
                f"console_command: av user %player% addpoints {milestone // 8}",
                f"console_command: voucher give randombp {i + 1} %player%" if i >= 3 else ""
            ]
...
def generate_gui_data(block_name, starting_number):
    ...
    for i, slot in enumerate(slots, start=10):
        reward_tokens = milestone_number // 8
        ...
        gui_data['items'][str(slot)] = {
            ...
            'lore': [
                ...
                f'  &f{reward_mystery_boxes}x Random Mystery Box' if i >= 3 else ""
            ]
        }
        milestone_number *= 2
...
def generate_quests_and_gui(starting_number):
    ...
    with open('mining.yml', 'w') as file:
